...
static int decode_residues(TAKDecContext *s, int32_t *decoded, int length)
{
    ...
    if (bitstream_read_bit(bc)) {
        ...
        coding_mode[0] = mode = bitstream_read(bc, 6);
...
static int decode_channel(TAKDecContext *s, int chan)
{
    ...
    s->sample_shift[chan] = get_bits_esc4(bc);
     ...
     *       non-bit-exact output. Reading the 1 bit using bitstream_read_signed() instead
