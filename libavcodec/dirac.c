                                   ...
                                   void *log_ctx)
{
    ...
    if (bitstream_read_bit(bc)) { /* [DIRAC_STD] custom_clean_area_flag */
        ...
        dsh->clean_left_offset = svq3_get_ue_golomb(bc);
        /* [DIRAC_STD] CLEAN_RIGHT_OFFSET */
        dsh->clean_right_offset = svq3_get_ue_golomb(bc);
    }
    ...
    if (bitstream_read_bit(bc)) { /* [DIRAC_STD] custom_signal_range_flag */
        /* [DIRAC_STD] index */
        dsh->pixel_range_index = svq3_get_ue_golomb(bc);
        ...
        if (!dsh->pixel_range_index) {
            luma_offset = svq3_get_ue_golomb(bc);
            luma_depth  = av_log2(svq3_get_ue_golomb(bc)) + 1;
            svq3_get_ue_golomb(bc); /* chroma offset    */
            svq3_get_ue_golomb(bc); /* chroma excursion */
    ...
    if (bitstream_read_bit(bc)) { /* [DIRAC_STD] custom_colour_spec_flag */
        /* [DIRAC_STD] index */
        idx = dsh->color_spec_index = svq3_get_ue_golomb(bc);
        ...
        if (!dsh->color_spec_index) {
            ...
            if (bitstream_read_bit(bc) && !svq3_get_ue_golomb(bc))
                                   ...
                                   void *log_ctx)
{
    ...
    version_major  = svq3_get_ue_golomb(&bc);
    svq3_get_ue_golomb(&bc); /* version_minor */
    dsh->profile = svq3_get_ue_golomb(&bc);
    dsh->level   = svq3_get_ue_golomb(&bc);
    ...
    video_format   = svq3_get_ue_golomb(&bc);
    ...
    picture_coding_mode = svq3_get_ue_golomb(&bc);
