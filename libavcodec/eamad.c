...
static inline void decode_block_intra(MadContext *s, int16_t * block)
{
    ...
    block[0] = (128 + bitstream_read_signed(&s->bb, 8)) * quant_matrix[0];
    ...
    i = 0;
    {
        ...
        for (;;) {
            BITSTREAM_RL_VLC(level, run, &s->bb, rl->rl_vlc[0], TEX_VLC_BITS, 2);
            ...
            if (level == 127) {
                ...
                level = (level-1)|1;
                level = (level ^ bitstream_peek_signed(&s->bb, 1)) - bitstream_peek_signed(&s->bb, 1);
                bitstream_skip(&s->bb, 1);
            } else {
                /* escape */
                level = bitstream_read_signed(&s->bb, 10);

                run = bitstream_read(&s->bb, 6) + 1;
...
static int decode_motion(BitstreamContext *bb)
{
    int value = 0;
...
static void decode_mb(MadContext *s, AVFrame *frame, int inter)
{
    ...
    if (inter) {
        int v = bitstream_decode210(&s->bb);
        if (v < 2) {
            mv_map = v ? bitstream_read(&s->bb, 6) : 63;
            mv_x = decode_motion(&s->bb);
            mv_y = decode_motion(&s->bb);
        } else {
            mv_map = 0;
        }
    }
                        ...
                        AVPacket *avpkt)
{
    ...
    bitstream_init8(&s->bb, s->bitstream_buf, bytestream2_get_bytes_left(&gb));
