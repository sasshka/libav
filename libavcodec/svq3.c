    ...
    for (i = 0; i < 16; i += part_height)
        for (j = 0; j < 16; j += part_width) {
            ...
            if (mode == PREDICT_MODE) {
                ...
                dy = svq3_get_se_golomb(&s->bc_slice);
                dx = svq3_get_se_golomb(&s->bc_slice);
...
static int svq3_decode_mb(SVQ3Context *s, unsigned int mb_type)
{
    ...
    if (mb_type == 0) {           /* SKIP */
        ...
        if (mb_type == 8) {
            ...
            for (i = 0; i < 16; i += 2) {
                vlc = svq3_get_ue_golomb(&s->bc_slice);
    ...
    if (!IS_INTRA16x16(mb_type) &&
        ...
        if ((vlc = svq3_get_ue_golomb(&s->bc_slice)) >= 48) {
    ...
    if (IS_INTRA16x16(mb_type) ||
        ...
        s->qscale += svq3_get_se_golomb(&s->bc_slice);
...
static int svq3_decode_slice_header(AVCodecContext *avctx)
{
    ...
    if (((header & 0x9F) != 1 && (header & 0x9F) != 2) || (header & 0x60) == 0) {
        ...
        memset(s->slice_buf + slice_bytes, 0, AV_INPUT_BUFFER_PADDING_SIZE);
        if (!s->slice_buf)
    ...
    slice_id = svq3_get_ue_golomb(&s->bc_slice);
    if ((slice_id) >= 3) {
...
static av_cold int svq3_decode_init(AVCodecContext *avctx)
{
    ...
    if (marker_found) {
        ...
        if (s->unknown_flag) {
#if CONFIG_ZLIB
            unsigned watermark_width  = svq3_get_ue_golomb(&bc);
            unsigned watermark_height = svq3_get_ue_golomb(&bc);
            int u1                    = svq3_get_ue_golomb(&bc);
            int u2                    = bitstream_read(&bc, 8);
            int u3                    = bitstream_read(&bc, 2);
            int u4                    = svq3_get_ue_golomb(&bc);
                             ...
                             int *got_frame, AVPacket *avpkt)
{
    ...
    for (s->mb_y = 0; s->mb_y < s->mb_height; s->mb_y++) {
        for (s->mb_x = 0; s->mb_x < s->mb_width; s->mb_x++) {
            ...
            mb_type = svq3_get_ue_golomb(&s->bc_slice);
