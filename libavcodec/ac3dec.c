...
static int parse_frame_header(AC3DecodeContext *s)
{
    ...
    err = ff_ac3_parse_header(&s->bc, &hdr);
    if (err)
        return err;
...
static inline int coupling_coordinates(AC3DecodeContext *s, int blk)
{
    ...
    for (ch = 1; ch <= fbw_channels; ch++) {
        if (s->channel_in_cpl[ch]) {
            if ((s->eac3 && s->first_cpl_coords[ch]) || bitstream_read_bit(bc)) {
    ...
    if (s->channel_mode == AC3_CHMODE_STEREO && cpl_coords_exist) {
        for (bnd = 0; bnd < s->num_cpl_bands; bnd++) {
            s->phase_flags[bnd] = s->phase_flags_in_use ? bitstream_read_bit(bc) : 0;
        }
    }
...
static int decode_audio_block(AC3DecodeContext *s, int blk)
{
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_INDEPENDENT && bitstream_read_bit(bc)) {
