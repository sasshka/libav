...
void ff_eac3_decode_transform_coeffs_aht_ch(AC3DecodeContext *s, int ch)
{
    ...
    if (gaq_mode == EAC3_GAQ_12 || gaq_mode == EAC3_GAQ_14) {
        ...
        for (bin = s->start_freq[ch]; bin < s->end_freq[ch]; bin++) {
            if (s->bap[ch][bin] > 7 && s->bap[ch][bin] < end_bap)
                gaq_gain[gs++] = bitstream_read_bit(bcc) << (gaq_mode-1);
        }
        ...
        for (bin = s->start_freq[ch]; bin < s->end_freq[ch]; bin++) {
            if (s->bap[ch][bin] > 7 && s->bap[ch][bin] < 17) {
                if (gc++ == 2) {
                    int group_code = bitstream_read(bcc, 5);
                    if (group_code > 26) {
    ...
    for (bin = s->start_freq[ch]; bin < s->end_freq[ch]; bin++) {
        ...
        if (!hebap) {
            ...
            for (blk = 0; blk < 6; blk++) {
                int mant = bitstream_read_signed(bcc, gbits);
                if (log_gain && mant == -(1 << (gbits-1))) {
                    /* large mantissa */
                    int b;
                    int mbits = bits - (2 - log_gain);
                    mant = bitstream_read_signed(bcc, mbits);
                    mant <<= (23 - (mbits - 1));
...
int ff_eac3_parse_header(AC3DecodeContext *s)
{
    ...
    bitstream_skip(bcc, 5); // skip bitstream id

    /* volume control params */
    for (i = 0; i < (s->channel_mode ? 1 : 2); i++) {
        bitstream_skip(bcc, 5); // skip dialog normalization
        if (bitstream_read_bit(bcc)) {
            bitstream_skip(bcc, 8); // skip compression gain word
        }
    }
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_DEPENDENT) {
        if (bitstream_read_bit(bcc)) {
            bitstream_skip(bcc, 16); // skip custom channel map
        }
    }
    ...
    if (bitstream_read_bit(bcc)) {
        ...
        /* lfe mix level */
        if (s->lfe_on && (s->lfe_mix_level_exists = bitstream_read_bit(bcc))) {
            s->lfe_mix_level = bitstream_read(bcc, 5);
        }
        ...
        if (s->frame_type == EAC3_FRAME_TYPE_INDEPENDENT) {
            for (i = 0; i < (s->channel_mode ? 1 : 2); i++) {
                ...
                if (bitstream_read_bit(bcc)) {
                    bitstream_skip(bcc, 6);  // skip program scale factor
                }
            }
            if (bitstream_read_bit(bcc)) {
                bitstream_skip(bcc, 6);  // skip external program scale factor
            }
            /* skip mixing parameter data */
            switch(bitstream_read(bcc, 2)) {
                case 1: bitstream_skip(bcc, 5);  break;
                case 2: bitstream_skip(bcc, 12); break;
                case 3: {
                    int mix_data_size = (bitstream_read(bcc, 5) + 2) << 3;
                    bitstream_skip(bcc, mix_data_size);
                    break;
                }
            }
                    ...
                    if (bitstream_read_bit(bcc)) {
                        /* note: this is not in the ATSC A/52B specification
                           reference: ETSI TS 102 366 V1.1.1
                                      section: E.1.3.1.25 */
                        bitstream_skip(bcc, 8);  // skip pan mean direction index
                        bitstream_skip(bcc, 6);  // skip reserved paninfo bits
                    }
                }
            }
            ...
            if (bitstream_read_bit(bcc)) {
                for (blk = 0; blk < s->num_blocks; blk++) {
                    if (s->num_blocks == 1 || bitstream_read_bit(bcc)) {
                        bitstream_skip(bcc, 5);
                    }
                }
            }
        }
    ...
    if (bitstream_read_bit(bcc)) {
        s->bitstream_mode = bitstream_read(bcc, 3);
        bitstream_skip(bcc, 2); // skip copyright bit and original bitstream bit
        if (s->channel_mode == AC3_CHMODE_STEREO) {
            s->dolby_surround_mode  = bitstream_read(bcc, 2);
        ...
        for (i = 0; i < (s->channel_mode ? 1 : 2); i++) {
            if (bitstream_read_bit(bcc)) {
                bitstream_skip(bcc, 8); // skip mix level, room type, and A/D converter type
            }
        }
        if (s->bit_alloc_params.sr_code != EAC3_SR_CODE_REDUCED) {
            bitstream_skip(bcc, 1); // skip source sample rate code
        }
    }
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_INDEPENDENT && s->num_blocks != 6) {
        bitstream_skip(bcc, 1); // skip converter synchronization flag
    }

    /* original frame size code if this stream was converted from AC-3 */
    if (s->frame_type == EAC3_FRAME_TYPE_AC3_CONVERT &&
            (s->num_blocks == 6 || bitstream_read_bit(bcc))) {
        bitstream_skip(bcc, 6); // skip frame size code
    }

    /* additional bitstream info */
    if (bitstream_read_bit(bcc)) {
        int addbsil = bitstream_read(bcc, 6);
        for (i = 0; i < addbsil + 1; i++) {
            bitstream_skip(bcc, 8); // skip additional bit stream info
        }
    }
    ...
    if (s->num_blocks == 6) {
        ac3_exponent_strategy = bitstream_read_bit(bcc);
        parse_aht_info        = bitstream_read_bit(bcc);
    } else {
    ...
    s->snr_offset_strategy    = bitstream_read(bcc, 2);
    parse_transient_proc_info = bitstream_read_bit(bcc);

    s->block_switch_syntax = bitstream_read_bit(bcc);
    if (!s->block_switch_syntax)
        memset(s->block_switch, 0, sizeof(s->block_switch));

    s->dither_flag_syntax = bitstream_read_bit(bcc);
    ...
    s->bit_allocation_syntax = bitstream_read_bit(bcc);
    ...
    s->fast_gain_syntax  = bitstream_read_bit(bcc);
    s->dba_syntax        = bitstream_read_bit(bcc);
    s->skip_syntax       = bitstream_read_bit(bcc);
    parse_spx_atten_data = bitstream_read_bit(bcc);

    /* coupling strategy occurrence and coupling use per block */
    num_cpl_blocks = 0;
    if (s->channel_mode > 1) {
        for (blk = 0; blk < s->num_blocks; blk++) {
            s->cpl_strategy_exists[blk] = (!blk || bitstream_read_bit(bcc));
    ...
    if (ac3_exponent_strategy) {
        ...
        for (blk = 0; blk < s->num_blocks; blk++) {
            for (ch = !s->cpl_in_use[blk]; ch <= s->fbw_channels; ch++) {
                s->exp_strategy[blk][ch] = bitstream_read(bcc, 2);
            }
        }
    } else {
        /* LUT-based exponent strategy syntax */
        for (ch = !((s->channel_mode > 1) && num_cpl_blocks); ch <= s->fbw_channels; ch++) {
            int frmchexpstr = bitstream_read(bcc, 5);
            for (blk = 0; blk < 6; blk++) {
    ...
    if (s->lfe_on) {
        for (blk = 0; blk < s->num_blocks; blk++) {
            s->exp_strategy[blk][s->lfe_ch] = bitstream_read_bit(bcc);
        }
    }
    /* original exponent strategies if this stream was converted from AC-3 */
    if (s->frame_type == EAC3_FRAME_TYPE_INDEPENDENT &&
            (s->num_blocks == 6 || bitstream_read_bit(bcc))) {
        bitstream_skip(bcc, 5 * s->fbw_channels); // skip converter channel exponent strategy
    }
    ...
    if (parse_aht_info) {
        ...
        for (ch = (num_cpl_blocks != 6); ch <= s->channels; ch++) {
            ...
            s->channel_uses_aht[ch] = use_aht && bitstream_read_bit(bcc);
        }
    } else {
    ...
    if (!s->snr_offset_strategy) {
        int csnroffst = (bitstream_read(bcc, 6) - 15) << 4;
        int snroffst = (csnroffst + bitstream_read(bcc, 4)) << 2;
    ...
    if (parse_transient_proc_info) {
        for (ch = 1; ch <= s->fbw_channels; ch++) {
            if (bitstream_read_bit(bcc)) { // channel in transient processing
                bitstream_skip(bcc, 10); // skip transient processing location
                bitstream_skip(bcc, 8);  // skip transient processing length
            }
        }
    }
    ...
    for (ch = 1; ch <= s->fbw_channels; ch++) {
        if (parse_spx_atten_data && bitstream_read_bit(bcc)) {
            s->spx_atten_code[ch] = bitstream_read(bcc, 5);
        } else {
    ...
    if (s->num_blocks > 1 && bitstream_read_bit(bcc)) {
        ...
        bitstream_skip(bcc, block_start_bits);
