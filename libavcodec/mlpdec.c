...
static inline int read_huff_channels(MLPDecodeContext *m, BitstreamContext *bcp,
                                     unsigned int substr, unsigned int pos)
{
    ...
    for (mat = 0; mat < s->num_primitive_matrices; mat++)
        if (s->lsb_bypass[mat])
            m->bypassed_lsbs[pos + s->blockpos][mat] = bitstream_read_bit(bcp);
    ...
    for (channel = s->min_channel; channel <= s->max_channel; channel++) {
        ...
        if (codebook > 0)
            result = bitstream_read_vlc(bcp, huff_vlc[codebook-1].table,
        ...
        if (lsb_bits > 0)
            result = (result << lsb_bits) + bitstream_read(bcp, lsb_bits);
...
static int read_restart_header(MLPDecodeContext *m, BitstreamContext *bcp,
                               const uint8_t *buf, unsigned int substr)
{
    ...
    int start_count = bitstream_tell(bcp);
    int min_channel, max_channel, max_matrix_channel;
    const int std_max_matrix_channel = m->avctx->codec_id == AV_CODEC_ID_MLP
                                     ? MAX_MATRIX_CHANNEL_MLP
                                     : MAX_MATRIX_CHANNEL_TRUEHD;

    sync_word = bitstream_read(bcp, 13);
    ...
    s->noise_type = bitstream_read_bit(bcp);
    ...
    bitstream_skip(bcp, 16); /* Output timestamp */

    min_channel        = bitstream_read(bcp, 4);
    max_channel        = bitstream_read(bcp, 4);
    max_matrix_channel = bitstream_read(bcp, 4);
    ...
    s->noise_shift   = bitstream_read(bcp,  4);
    s->noisegen_seed = bitstream_read(bcp, 23);

    bitstream_skip(bcp, 19);

    s->data_check_present = bitstream_read_bit(bcp);
    lossless_check = bitstream_read(bcp, 8);
    ...
    bitstream_skip(bcp, 16);

    memset(s->ch_assign, 0, sizeof(s->ch_assign));

    for (ch = 0; ch <= s->max_matrix_channel; ch++) {
        int ch_assign = bitstream_read(bcp, 6);
        if (m->avctx->codec_id == AV_CODEC_ID_TRUEHD) {
    ...
    checksum = ff_mlp_restart_checksum(buf, bitstream_tell(bcp) - start_count);

    if (checksum != bitstream_read(bcp, 8))
...
static int read_filter_params(MLPDecodeContext *m, BitstreamContext *bcp,
                              ...
                              unsigned int filter)
{
    ...
    if (order > 0) {
        ...
        fp->shift = bitstream_read(bcp, 4);

        coeff_bits  = bitstream_read(bcp, 5);
        coeff_shift = bitstream_read(bcp, 3);
        ...
        for (i = 0; i < order; i++)
            fcoeff[i] = bitstream_read_signed(bcp, coeff_bits) << coeff_shift;

        if (bitstream_read_bit(bcp)) {
            ...
            state_bits  = bitstream_read(bcp, 4);
            state_shift = bitstream_read(bcp, 4);

            /* TODO: Check validity of state data. */

            for (i = 0; i < order; i++)
                fp->state[i] = bitstream_read_signed(bcp, state_bits) << state_shift;
        }
    }
...
static int read_matrix_params(MLPDecodeContext *m, unsigned int substr, BitstreamContext *bcp)
{
    SubStream *s = &m->substream[substr];
    unsigned int mat, ch;
    ...
    s->num_primitive_matrices = bitstream_read(bcp, 4);
    ...
    for (mat = 0; mat < s->num_primitive_matrices; mat++) {
        ...
        s->matrix_out_ch[mat] = bitstream_read(bcp, 4);
        frac_bits             = bitstream_read(bcp, 4);
        s->lsb_bypass   [mat] = bitstream_read_bit(bcp);
        ...
        for (ch = 0; ch <= max_chan; ch++) {
            int coeff_val = 0;
            if (bitstream_read_bit(bcp))
                coeff_val = bitstream_read_signed(bcp, frac_bits + 2);

            s->matrix_coeff[mat][ch] = coeff_val << (14 - frac_bits);
        }

        if (s->noise_type)
            s->matrix_noise_shift[mat] = bitstream_read(bcp, 4);
        else
...
static int read_channel_params(MLPDecodeContext *m, unsigned int substr,
                               BitstreamContext *bcp, unsigned int ch)
{
    SubStream *s = &m->substream[substr];
    ChannelParams *cp = &s->channel_params[ch];
    ...
    if (s->param_presence_flags & PARAM_FIR)
        if (bitstream_read_bit(bcp))
            if ((ret = read_filter_params(m, bcp, substr, ch, FIR)) < 0)
                return ret;

    if (s->param_presence_flags & PARAM_IIR)
        if (bitstream_read_bit(bcp))
            if ((ret = read_filter_params(m, bcp, substr, ch, IIR)) < 0)
                return ret;
    ...
    if (s->param_presence_flags & PARAM_HUFFOFFSET)
        if (bitstream_read_bit(bcp))
            cp->huff_offset = bitstream_read_signed(bcp, 15);

    cp->codebook  = bitstream_read(bcp, 2);
    cp->huff_lsbs = bitstream_read(bcp, 5);
...
static int read_decoding_params(MLPDecodeContext *m, BitstreamContext *bcp,
                                unsigned int substr)
{
    SubStream *s = &m->substream[substr];
    unsigned int ch;
    ...
    if (s->param_presence_flags & PARAM_PRESENCE)
        if (bitstream_read_bit(bcp))
            s->param_presence_flags = bitstream_read(bcp, 8);

    if (s->param_presence_flags & PARAM_BLOCKSIZE)
        if (bitstream_read_bit(bcp)) {
            s->blocksize = bitstream_read(bcp, 9);
    ...
    if (s->param_presence_flags & PARAM_MATRIX)
        if (bitstream_read_bit(bcp))
            if ((ret = read_matrix_params(m, substr, bcp)) < 0)
                return ret;

    if (s->param_presence_flags & PARAM_OUTSHIFT)
        if (bitstream_read_bit(bcp)) {
            for (ch = 0; ch <= s->max_matrix_channel; ch++)
                s->output_shift[ch] = bitstream_read_signed(bcp, 4);
            if (substr == m->max_decoded_substream)
                m->dsp.mlp_pack_output = m->dsp.mlp_select_pack_output(s->ch_assign,
                                                                       s->output_shift,
                                                                       s->max_matrix_channel,
    ...
    if (s->param_presence_flags & PARAM_QUANTSTEP)
        if (bitstream_read_bit(bcp))
            for (ch = 0; ch <= s->max_channel; ch++) {
                ...
                s->quant_step_size[ch] = bitstream_read(bcp, 4);
    ...
    for (ch = s->min_channel; ch <= s->max_channel; ch++)
        if (bitstream_read_bit(bcp))
            if ((ret = read_channel_params(m, substr, bcp, ch)) < 0)
                return ret;
...
static int read_block_data(MLPDecodeContext *m, BitstreamContext *bcp,
                           unsigned int substr)
{
    SubStream *s = &m->substream[substr];
    ...
    if (s->data_check_present) {
        expected_stream_pos  = bitstream_tell(bcp);
        expected_stream_pos += bitstream_read(bcp, 16);
        avpriv_request_sample(m->avctx,
    ...
    for (i = 0; i < s->blocksize; i++)
        if ((ret = read_huff_channels(m, bcp, substr, i)) < 0)
            return ret;
    ...
    if (s->data_check_present) {
        if (bitstream_tell(bcp) != expected_stream_pos)
            av_log(m->avctx, AV_LOG_ERROR, "block data length mismatch\n");
        bitstream_skip(bcp, 8);
    }
